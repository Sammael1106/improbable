...
const viewRatio = windowHeight/windowWidth
const viewVertical = viewRatio > 1
let zoom = 65
if (windowWidth <= 1200) zoom = 60
if (windowWidth <= 1024) zoom = 57
if (windowWidth <= 900) zoom = viewVertical ? 75 : 35
if (windowWidth <= 500) zoom = 45

console.log(zoom, windowWidth, windowHeight, viewVertical)
export default function Home() {
  ...
  const changeStage = (newstage, camera) => {
  ...
  return (
    <>
      <div  className={clsx(styles.page, styles[`stage${stage}`], styles[`zoom${config.zoom}`])}>
      ...
      <div className={clsx(styles[`stage${stage}`], styles.pageIndicator)}>
...
const StageOne = ({ config, setStage }) => {
  ...
  return (
    <>
      ...
      <Html position={viewVertical ? [-3, 0, -6] : [-6, 0, -6]} className={viewVertical ? styles.doubleCell : styles.cell} distanceFactor={0.01555}>
      ...
      <Html position={viewVertical ? [-3, 0, 3] : [3, 0, 0]} className={styles.doubleCell} distanceFactor={0.01555}>
        ...
        <div className={viewVertical ? styles.arrowRight : styles.arrowLeft} onClick={() => setClicked(true)}><span>let's dive deeper</span></div>
      </Html>
...
const StageTwo = ({ config, setStage, stage }) => {
  ...
  return (
    <ScrollControls pages={1}>
      ...
      {/* <Grid /> */}
      <Html position={viewVertical ? [-3, 0, -1]:[-9, 0, -2]} className={styles.trippleCell}>
        <h2 className={`${styles.heading} ${styles.alignBottom}`}><Heading>Born in the Digital Realm</Heading></h2>
        <p  style={{marginTop: 32}}>I discovered my passion for digital worlds at an early age, a passion that led me to a career in web development and visual design. Today, I use my skills to create immersive experiences, believing that these worlds can provide us with a sense of fulfillment and endless opportunities for creativity.</p>
        <div className={styles.row}>
          <div className={`${styles.arrowLeft} ${styles.inactive}`} onClick={() => setStage(1, camera)}><span>Back</span></div>
          <div className={styles.arrowRight} onClick={() => setStage(3, camera)}><span>Next</span></div>
        </div>
      </Html>
      <Html position={viewVertical ? [-3, 0, viewport.height - 6 ] : [0, 0, viewport.height - 3]} className={styles.trippleCell}>
        <h2 className={`${styles.heading} ${styles.alignBottom} ${styles.alignRight}`}>Skills and Expertise to Navigate the Metaverse</h2>
        <p style={{marginTop: 32}} className={`${styles.alignRight}`}>Throughout my career, I've honed a diverse set of skills, from JavaScript to Unreal Engine, to bring my visions to life. I specialize in optimizing 2D & 3D assets, implementing them into game engines, and designing unique user interfaces.</p>
        <div className={styles.row} >
          <div className={`${styles.arrowLeft} ${styles.inactive}`} style={{marginLeft: 'auto'}} onClick={() => setStage(2, camera)}><span>Back</span></div>
          <div className={styles.arrowRight} onClick={() => setStage(4, camera)}><span>Next</span></div>
        </div>
      </Html>
    </ScrollControls>
  )
...
const ScrollableGroups = ({materialPoperties}) => {
  ...
  return (
    <>
      <group position={viewVertical ? [-3,0,-2] : [0,0,0]} ref={floatingGroup}>
      ...
      <group position={viewVertical ? [3,0,3] : [0,0,0]} ref={imagesGroup}>
        <Image url="images/bug.png" rotation={[Math.PI/-2, 0, 0]} scale={8}  position={[-3, -5, viewport.height/2]} transparent={true} opacity={0}/>
        <Image url="images/vscode.png" rotation={[Math.PI/-3, 0, 1]} scale={10} position={[-6, -8, viewport.height/-2 + 3]} transparent={true} opacity={0}/>
      </group>
    </>
  )
}
...
function StageFour({ config, setStage }) {
  ...
  return (
    <group dispose={null}>
      <Html position={viewVertical ? [-3, 0, -8] : [-6, 0, -6]} className={viewVertical ? styles.doubleCell : styles.cell} style={{marginLeft: viewVertical ? 0 : -40}}>
        <h1 className={`${styles.heading} ${styles.alignRight} ${styles.alignBottom}`}>Why Improbable?</h1>
      </Html>
      <Html position={viewVertical ? [-3, 0, 1] : [3, 0, -3.3]} className={styles.doubleCell} >
        <h3 className={`${styles.heading} ${styles.alignBottom}`}><Heading>An Alignment of Visions</Heading></h3>
        <p style={{marginTop: 32}}>Improbable's commitment to developing deeply immersive, interconnected virtual worlds resonates with my own beliefs. Your pioneering work in the field, specifically the Morpheus technology, inspires me, and I'm excited to potentially contribute to your efforts.</p>
        <div className={styles.row} >
          <div className={`${styles.arrowLeft} ${styles.inactive}`} onClick={() => setStage(3, camera)}><span>Back</span></div>
          <div className={styles.arrowRight} onClick={() => setStage(5, camera)}><span>Next</span></div>
        </div>
      </Html>
      <Center>
        <mesh
          position={viewVertical ? [0,0,-2] : [0,0,0]}
          scale={3}
          castShadow
          geometry={nodes.Cylinder.geometry}
          onClick={() => setStage(5, camera)}
          >
            <meshBasicMaterial color={'white'} />
        </mesh>
        <mesh scale={40} rotation={[Math.PI/-3,0,0]} position={[0, -5, 0]} ref={background}>
          <planeGeometry />
...
const StageFive = ({ config, setStage, stage }) => {
  ...
  return (
    <>
      <color attach="background" args={['#000']} />
      {/* <Grid /> */}
      <Html position={viewVertical ? [-3, 0, -3]:[-9, 0, -2]} className={styles.trippleCell} distanceFactor={0.01555}>
        <h2 className={`${styles.heading} ${styles.alignBottom}`}><Heading>Thank you</Heading></h2>
        <p  style={{marginTop: 32}}>Let me thank you for the opportunity to be considered as a candidate for Improbable. I invite you to reach out to me. Let's explore how we can build unforgettable experiences together.</p>
        <div className={styles.row}>
          <div className={`${styles.arrowLeft} ${styles.inactive}`} style={{marginRight: 24}} onClick={() => setStage(4, camera)}><span>Back</span></div>
